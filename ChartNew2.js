/*
ChartNew.js
Better styling, using better JS practices.
*/
  ...
  function animationLoop(config, drawScale, drawData, ctx, clrx, clry, clrwidth, clrheight, midPosX, midPosY, borderX, borderY, data) {
    ...
    function animLoop() {
      ...
      if (multAnim == -1 && cntiter <= beginAnim) {
        if (typeof config.onAnimationComplete == "function") config.onAnimationComplete(ctx, config, data, 0, animationCount);
        multAnim = 1;
        requestAnimFrame(animLoop);
      } else if (percentAnimComplete < config.animationStopValue) {
        requestAnimFrame(animLoop);
      } else {
        if (typeof config.onAnimationComplete == "function") config.onAnimationComplete(ctx, config, data, 1, animationCount);
        // stop animation ? 
      ...
      for (var i = fromi - 1; i >= 0; i--) {
        ...
        if (tpof == "string") {
          ...
          if (lgtxt != "") {
            ...
            if (drawLegendOnData) {
              if (typeof data.datasets[orderi].strokeColor == "string") ctx.strokeStyle = data.datasets[orderi].strokeColor;
              else ctx.strokeStyle = config.defaultStrokeColor;
            } else {
              if (typeof data[orderi].color == "string") ctx.strokeStyle = data[orderi].color;
              else ctx.strokeStyle = config.defaultStrokeColor;
            }
            ...
            if (legendBox) {
              ...
              ctx.closePath();
              if (drawLegendOnData) {
                if (typeof data.datasets[orderi].fillColor == "string") ctx.fillStyle = data.datasets[orderi].fillColor;
                else ctx.fillStyle = config.defaultFillColor;
              } else {
